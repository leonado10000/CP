...
"""    count = 0
    ...
    for x in range(0,len(ideas)-1):
        if ideas[x][0] == ideas[x+1][0]:
            ...
            elle.append(x+1)
    print(elle,ideas)
    for x in range(len(ideas)):
        for y in elle:
            if ideas[y][0] + ideas[x][1:] in ideas or ideas[x][0] + ideas[y][1:] in ideas:
                continue
            else:
                print(ideas[x],ideas[y])
                count+=2
    return count """
...
print(distinctNames(0,ideas =["coffee","donuts","time","toffee"]))
